...
<h3><span class="glyphicon glyphicon-list"></span> Data Menu</h3>
<a style="margin-bottom:20px" class="btn btn-info col-md-2" href="tambah_barang.php"><span class="glyphicon glyphicon-plus"></span>Tambah Menu</a>
<br/>
<br/>
...
<form action="cari_produk.php" method="GET">
	<div class="input-group col-md-5 col-md-offset-7">
		...
		<input type="text" class="form-control" placeholder="Cari berdasarkan ID Menu, nama dan jumlah di sini .." aria-describedby="basic-addon1" name="cari">
	</div>
</form><br />
	<?php 
	if(isset($_GET['cari'])){
		...
		$brg=mysqli_query($koneksi, "select * from makanan inner join kategori on makanan.id_kategori = kategori.id_kategori where nama like '%$cari%' or qty like '%$cari%' or id_produk like '%$cari%'");
	}else{
		$brg=mysqli_query($koneksi, "select * from makanan inner join kategori on makanan.id_kategori = kategori.id_kategori order by nama DESC limit $start, $per_hal");
	...
	if($count == null){
		if(isset($_GET['cari'])){
			echo '<div align="center"> <h5>Menu dengan kata kunci "'. $_GET['cari'] .'" tidak ada. </h5> </div>';
		}
	}else{
	?>
	...
	<tr>
		...
		<?php 
		if (isset($cari)) {
			$x=mysqli_query($koneksi, "SELECT sum(harga) as total from makanan where nama like '%$cari%'");
		}else{
