<?php
    include 'header.php';
    $id_produk = $_GET['id'];
    $makanan = mysqli_query($koneksi, "SELECT * FROM makanan WHERE id_produk = '$id_produk'") or die(mysqli_error($koneksi));
...
<div class="col-md-9 contact_left">
    <form action="komen_proses.php?id=<?=$id_produk?>" method="post">
					<div class="column_3">
	                   <textarea value="Message" style="font-size: 20px;" name="komentar" placeholder="" onfocus="this.value = '';" onblur="if (this.value == '') {this.value = 'Masukkan komentar anda tentang menu ini';}">Masukkan komentar anda tentang menu ini</textarea>
	                </div>
