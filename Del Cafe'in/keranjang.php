...
<div class="container">
	<div class="check">	 
		<div class="col-md-9 cart-items">
                    <ul class="back">
                	  <li style="font-size: 16px; margin-left: -13px;"><i class="back_arrow"> </i>Pesan lagi di <a href="produk.php">Halaman Menu Makanan</a></li>
                    </ul>
			 <h1>Keranjang Pesanan Makanan</h1>
                        
                        <?php
                $total = 0;
                if (isset($_SESSION['items'])) {
                    foreach ($_SESSION['items'] as $key => $val) {
                        $query = mysqli_query($koneksi, "select * from makanan where id_produk = '$key'");
                        $data = mysqli_fetch_array($query);
                        $jumlah_harga = $data['harga'] * $val;
                        $total += $jumlah_harga;
                        $makanan = $data['id_produk'];
                        $jumlah = $val;
		 ...
		 <div class="col-md-3 cart-total">
			 ...
			 <div class="clearfix"></div>
		 <a class="continue" href="pembayaran.php?total_harga=<?=$total?>&jumlah=<?=$jumlah?>&id_produk=<?=$makanan?>">Continue</a>
			</div>
	 </div>
</div>
